...
var Modal_crop = {
    ...
    elements : function(){
        ...
        this.cropInstance = $("#box_image");
        this.confirmCrop  = $("[data-action='js-confirm-generic']");
    ...
    events : function(){
        ...
        this.modal.on("openModalCrop", function(e,data){
            self.openModal.call(self,data);
        });

        // aggiungo i metodi
        this.modal.on('shown.bs.modal', function(e){

            self.cropInstance.cropper("destroy");
            
            if( self.currentConfiguration.imageWidth < self.currentConfiguration.cropdimensionwidth && self.currentConfiguration.imageHeight < self.currentConfiguration.cropdimensionheight ){

                Notify( NotificationError.searchError("warningerr001") , 'bottom-right', '5000', NotificationError.typeError("warningerr001"), 'fa-power-off', true);
                self.cropperAlert = true;
            }

            self.cropInstance.cropper({ 
                checkCrossOrigin: false,
                viewMode: 2,
                minCanvasHeight: self.currentConfiguration.imageHeight,
                aspectRatio: self.currentConfiguration.cropratio,
                scalable: false,
                zoomable: false
            });
            
            self.cropInstance.on("built.cropper", function(e){
                // 
                self.cropInstance.cropper("setCanvasData", {
                    aspectRatio: self.currentConfiguration.cropratio
                });

                self.cropInstance.cropper("setData", {
                    width: self.currentConfiguration.cropdimensionwidth,
                    height: self.currentConfiguration.cropdimensionheight
                });

            });

            self.cropInstance.on("crop.cropper", function(event){
                if(self.currentConfiguration.cropdimensionwidth > event.width && self.currentConfiguration.cropdimensionheight > event.height){
                    self.cropInstance.cropper("setData", {
                        width: self.currentConfiguration.cropdimensionwidth,
                        height: self.currentConfiguration.cropdimensionheight
                    });
        ...
        this.confirmCrop.on("click", function(){

            self.currentConfiguration.instance.trigger("CropIsSelected",{
                cropperData : self.cropInstance.cropper("getData", true) , 
                cropperItem : self.cropInstance.cropper("getCroppedCanvas"),
                cropperAlert: self.cropperAlert
            });
    ...
    openModal : function(data,el){
        this.currentConfiguration = data.inputValues;
...
var UploadFormField = function(el) {
    this.ui(el);
    this.init();
    this.createDataObject();
    
    if( this.inputfileImage.length ){
        this.inputfileImage.after(this.createControls());
    } else if ( this.inputfileName.text().trim().length > 0 ) {

        this.inputfileName.append(this.createControls());
    }
    this.observe();
};    
UploadFormField.prototype.init = function (el){
    var self = this ;
    this.modal             = $("#modal-crop");
    this.imagebox          = this.modal.find("#box_image");
    this.btn               = this.elements.find("[data-action='js-upload-inform']");
    this.imageExtension    = ["jpg", "jpeg", "tiff", "bmp", "png"];
    this.elementObj        = "";
};

UploadFormField.prototype.createControls = function(){
    
    var self = this;

    this.clearfield = $("<span />", { class: "btn cancel" , click: function(event){ self.resetFields () } });
    this.clearfieldIcon = $("<i />", { class: "fa fa-times" }).appendTo(this.clearfield);

    return this.clearfield;
}
UploadFormField.prototype.ui = function(el){
    this.elements          = $(el);
    this.inputfileImage    = this.elements.find(".inform-image");
    this.inputfileName     = this.elements.find(".inform-name");
    this.thumb             = this.elements.find(".inform-thumb");
    this.uploadBar         = this.elements.find(".uploadstatus");
    this.inputDetails      = this.elements.find(".inform-details");
    this.cropElement       = "";
};
UploadFormField.prototype.observe = function (){
    var self = this, configuration;

    configuration = this.elements.data("configuration");

    this.elements.on("CropIsSelected", $.proxy( this.crop , this ) );

    this.inputfileImage.on("click", $.proxy( this.resetFields , this ) );

    this.elements.fileupload({
        "dataType": 'json',
        "autoUpload": true,
        "dropZone": self.elements,
        "url": globalVar.wsHandler,
        add: function(e, data){
            data.url = globalVar.wsHandler + '?UidCmsRepository=' + globalVar.default_Repository + '&UidCmsRepositoryFolder=' + globalVar.default_Folder;
            self.uploadBar.addClass("is-active");
            data.submit();
        },
        success: function(data){
            ...
            self.uploadBar.removeClass("is-active");

            if( self.imageExtension.indexOf( data[0].extension.toLowerCase() ) < 0 ){

                if( data[0].original != null ){
                    if(data[0] != "undefined"){
                        self.inputfileName.text(data[0].name);

                        // add reset button just create the interface
                        self.inputfileName.append(self.createControls());
                    }
                }

            }else{
                // control and inser image dimension
                var fakeimg =  new Image();
                    fakeimg.src = data[0].original;
                
                fakeimg.onload = function(){
                    configuration.imageWidth = this.width;
                    configuration.imageHeight = this.height;
                }
                //
                if( data[0].original != null ){

                    if (configuration.croptype == "false"){

                        if(data[0] != "undefined"){
                            self.inputfileName.text( data[0].name );
                            if( self.elements.find(".inform-image").length ){
                                self.elements.find(".inform-image").attr("src", data[0].original );    
                            }else{
                                var image = createFileImage (data[0].original);
                                self.thumb.prepend(image);
                                self.createControls();
                                image.after(self.createControls());
                        ...
                        // spedisco la configurazione al crop
                        $("#modal-crop").trigger("openModalCrop", {
                            "inputValues": configuration
                        });
                    }
                }else{

                    Notify('The folder response with original null', 'bottom-right', '5000', 'warning', 'fa-power-off', true);
...
UploadFormField.prototype.createDataObject = function(){

    var 
        splitElement = this.btn.attr("data-configuration"),
        obj
    ;
    if (splitElement.length){
    if(splitElement.indexOf("|") != -1){

        var configurationdata = splitElement.split("|");

        if( configurationdata[0] == "true"){
            obj = {
                instance            : this.elements,
                croptype            : configurationdata[0],
                cropratio           : configurationdata[1] == "true" ? parseInt( configurationdata[2]) / parseInt( configurationdata[3] ) : false,
                cropdimensionwidth  : parseInt( configurationdata[2] ),
                cropdimensionheight : parseInt( configurationdata[3] ),
                inputDimensionID    : this.btn.data("crop"),
                inputUrlID          : this.btn.data("cropurl")
            }
        }else{
            obj = {
                instance            : this.elements,
                croptype            : configurationdata[0],
                activefolder        : configurationdata[1],
                inputDimensionID    : this.btn.data("crop"),
                inputUrlID          : this.btn.data("cropurl")
            }
        }
    ...
    this.elements.data( "configuration", obj );

    }

};
UploadFormField.prototype.resetFields = function(){

    var configuration = this.elements.data("configuration");
    
    $("#" + configuration.inputDimensionID).val( "" );
    $("#" + configuration.inputUrlID).val( "TODELETE" );
    
    // se esiste l'immagine la cancello
    if( this.elements.find(".inform-image").length ){
        this.elements.find(".inform-image").remove();
        
        this.inputDetails.removeClass("error");
        configuration.instance.find("canvas").remove();
    } else {
        this.clearfield.remove();
    }
...
UploadFormField.prototype.wrapper = function(){
    var self = this;
    var configuration = this.elements.data("configuration");

    if(configuration.croptype == "true"){

        var imageUrlPath = this.lastElement[0].original;

        this.imagebox
            .css({"max-width" : "100%"})
            .attr("src", imageUrlPath )
        ;

        this.modal.trigger("openFromForm", { inputValues : configuration });

    }else{

        $( "#" + configuration.inputUrlID ).val( this.lastElement[0].original );

    }
};
UploadFormField.prototype.crop = function(event,data){

    var cropvalue, dataCrop, configuration, currentFormElement, inputDimension, inputUrl, name, image, item, alert;

    dataCrop = data.cropperData;
    item     = data.cropperItem;
    alert    = data.cropperAlert;

    // inserisci i dati per il salvataggio nei file corretti
    configuration = this.elements.data("configuration");
    cropvalue     = dataCrop["x"]+"|"+dataCrop["y"]+"|"+dataCrop["width"] +"|"+ dataCrop["height"];

    inputDimension = $("#" + configuration.inputDimensionID);
    inputUrl = $("#" + configuration.inputUrlID);
    currentFormElement = inputDimension.parents(".inform-upload");

    inputDimension.val( cropvalue );
    inputUrl.val( $("#box_image").attr("src") );    

    //
    image = currentFormElement.find(".inform-image");
    //
    ...
    if(alert){
        this.inputDetails.addClass("error");
    }else{
        this.inputDetails.removeClass("error");
    }

    // construct the image
    var div = $("<div/>" ,{ class: "inform-image" });

    $(item).appendTo(div);
    this.thumb.append(div);

    if( typeof this.clearfield == "undefined" ){
        this.thumb.append(this.createControls());
        console.log("aaa");
    }else{
    ...
    name = $("#box_image").attr("src").split("/");

    currentFormElement.find(".inform-name").text( name[name.length -1] );
...
if ($("#modal-crop").length) {
    Modal_crop.init();
}
$(".inform-upload").each(function (i, el) {
    if ($(el).find('[data-action="js-uploadfolder"]').length) {
    } else {
        var uploadFormField = new UploadFormField(el);
    }

});


function createFileImage (path){
    var image = $("<img />",{
        "class": "inform-image",
        "src": path
    });
