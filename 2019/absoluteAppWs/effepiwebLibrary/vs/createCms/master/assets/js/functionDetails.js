...
function InitAction(_divList) {
    ...
    $('#' + _divList + " .lnkAction").on('click', function () {
        ...
        if (dataAction == "deleteRelated")
            message = "Sei sicuro di voler ELIMINARE questo record?";
        if (dataAction == "enablerelated")
            message = "Sei sicuro di voler ABILITARE questo record?";
        if (dataAction == "disablerelated") 
            message = "Sei sicuro di voler DISABILITARE questo record?";
        if (dataAction == "disable")
            message = "Sei sicuro di voler SALVARE IN BOZZA questo record?";
        if (dataAction == "enable")
            message = "Sei sicuro di voler SALVARE E PUBBLICARE questo record?";
        ...
        if (dataAction == "deleteRelated")
            message = "Do you really want to Delete selected records?";
        if (dataAction == "enablerelated")
            message = "Do you really want to Enable selected records?";
        if (dataAction == "disablerelated") 
    ...
    $("#" + _divList + ' .lnkActionList').on('click', function (event) {
        ...
        if (dataAction == "enablerelated") {
            message = "Sei sicuro di voler ABILITARE i record selezionati?";
            returnConfirm = "deleterelated";
        }
        if (dataAction == "disablerelated") {
            message = "Sei sicuro di voler DISABILITARE i record selezionati?";
            returnConfirm = "deleterelated";
        }
        ...
        */
        ...
        function callService(selected, data, increment) {
            ...
            $.ajax({
                ...
                // console.info("inc= ", increment, "last= ", lastPosition - 1)
                if (increment < (lastPosition - 1)) {
                    ...
                    if (typeof callService === "function") {
                        if (response.trim() == "SUCCESS") {
                            ...
                            Notify(response, 'bottom-right', '3000', 'warning', 'fa-warning', true);
                        } else if (response.trim() == "ERROR") {
                            Notify(response, 'bottom-right', '3000', 'error', 'fa-bolt', true);
                        }
                        callService(selected, data, increment);
                    }
...
function InitActionNext(_divList, _contentTable) {
    var currentItemIndex = $('#' + _divList + " .relatedActive").index();
    var nextItemIndex = currentItemIndex + 1;
    ...
    if (nextItemIndex < $('#' + _divList).find('.card').length) {
        nextItemUid = $('#' + _divList).find('.card').eq(currentItemIndex + 1).attr("data-uid");
        $('#HiddenField_' + _contentTable + '_NextUid').val(nextItemUid);
        $('#Button_Related_' + _contentTable + '_SaveAndNext').removeClass("disabled");
    }
}